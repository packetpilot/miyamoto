...
PAYLOAD=\
	pack_manifests \
	puppet_environment_version \
	puppet_environment_flavor \
	pack_puppet_engine \
...
pack_manifests: manifest_d
	@sudo rsync -av --progress --stats nodeless_site.pp modules ${MANIFEST_D}
	@sudo chown -R root:admin ${MANIFEST_D}

pack_puppet_engine: pack_aws \
	pack_install_dmg_from_s3 \
	pack_mm_sync_facts \
	pack_mm_update_puppet_manifests \
	pack_puppet_plist \
	pack_puppetstarter
...
pack_mm_sync_facts: l_usr_local_sbin
	@sudo ${CP} ${PUPPET_BASE}/mm_sync_facts ${WORK_D}/usr/local/sbin
	@sudo chown root:admin ${WORK_D}/usr/local/sbin/mm_sync_facts
	@sudo chmod 755 ${WORK_D}/usr/local/sbin/mm_sync_facts

pack_mm_update_puppet_manifests: l_usr_local_sbin
	@sudo ${CP} ${PUPPET_BASE}/mm_update_puppet_manifests ${WORK_D}/usr/local/sbin
	@sudo chown root:admin ${WORK_D}/usr/local/sbin/mm_update_puppet_manifests
	@sudo chmod 755 ${WORK_D}/usr/local/sbin/mm_update_puppet_manifests
